...
// Combat talents (Spheres of Might): approximated with the same "1 per
// level + 2 bonus at first" pattern as magic talents. The real rule ties
// talent gain to each class's own Combat Training table, which roughly
// mirrors BAB progression but varies per class - treat this as a
// reasonable starting estimate, not a verified table.
...
// Skill talents (Spheres of Guile): the real rule ties talent gain to an
// operative's Expertise Tier (based on ranks in the associated skill, not
// class level directly). This is approximated with the same per-level
// pattern for now - treat as a rough placeholder, not a verified table.
