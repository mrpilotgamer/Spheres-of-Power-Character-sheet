...
Classes are split across three files by system - add to whichever matches:

- `src/data/classes.json` - Spherecaster classes (Power)
- `src/data/mightClasses.json` - Champion classes (Might)
- `src/data/guileClasses.json` - Operative classes (Guile)

All three share the same shape. Add an object like:
...
- `category`: the display grouping shown in the class dropdown - `"Spherecaster"`,
  `"Champion"`, or `"Operative"` for the three base systems.
- `system`: `"power"`, `"might"`, or `"guile"` - this is what the engine uses to route
  talent-budget and DC math, so it must match the folder/file you're adding to.
